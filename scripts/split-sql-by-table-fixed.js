...
// Function to extract SQL for a specific table section based on marker pattern
function extractTableSection(sql, number) {
  // Define the marker for this table section
  const startMarker = `-- ${number}. Create`;
  
  // Define the end marker (either the next table section or end of file)
  const endMarker = number < 5 ? `-- ${number + 1}. Create` : null;
  
  // Find the start position
  const startPos = sql.indexOf(startMarker);
  if (startPos === -1) {
    console.error(`Could not find section for table marker: ${startMarker}`);
    return '';
  }
...
function main() {
  try {
    console.log('======================================');
    console.log('Split SQL By Table (Fixed)');
    console.log('======================================');
    ...
    for (const section of tableSections) {
      ...
      const tableSql = extractTableSection(sql, section.number);
      
      if (tableSql) {
        // Write to file
        const outputFile = path.join(outputDir, `${section.number.toString().padStart(2, '0')}_${section.name}.sql`);
...
`;
    ...
    tableSections.forEach(section => {
      ...
      executionOrderScript += `-- Run: /migrations/output/tables/${section.number.toString().padStart(2, '0')}_${section.name}.sql\n`;
    });
