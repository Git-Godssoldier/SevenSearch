...
import { WebScrapingInputSchema, ExtractedPageContentOutputSchema } from '../mastra-schemas';
...
export const scrapeWebpageStep = createStep({
  name: 'ScrapeWebpageStep',
  inputSchema: WebScrapingInputSchema,
  outputSchema: ExtractedPageContentOutputSchema,
  execute: async ({ data, context }) => {
    try {
      const { targetUrl, originalQuery } = data;
      const { SCRAPYBARA_API_KEY, ANTHROPIC_API_KEY, pushUpdateToClient } = context;
      ...
      // Notify client about scraping start
      pushUpdateToClient({
        step: 3,
        type: "reading_source",
        payload: { 
      ...
      while (retryCount <= MAX_RETRIES) {
        try {
          ...
          // Log retry attempt
          console.warn(`[Scrape Webpage Step] Retry ${retryCount}/${MAX_RETRIES} for ${targetUrl}: ${error.message}`);
      ...
      return {
        ...
        title: extractTitle(scrapedContent),
        error: false
      };
    } catch (error) {
      console.error(`Error scraping webpage ${data.targetUrl}:`, error);
      ...
      return {
        link: data.targetUrl,
        content: [`Failed to extract content from ${data.targetUrl}. Error: ${error instanceof Error ? error.message : "Unknown error"}`],
        error: true
      };
    }
  }
