...
export const planningAndQueryEnhancementStep = createStep({
  ...
  description: 'Analyzes and enhances user queries for better search results',
  inputSchema: planningInput,
  outputSchema: planningOutput,
  async execute({ inputData, mastra, runtimeContext, emitter }) {
    const { originalQuery } = inputData;
    const { GEMINI_API_KEY, searchId, userId } = runtimeContext.getAll();

    console.log(`[Step: planning-query-enhancement] Started for searchId: ${searchId}, query: "${originalQuery}"`);
    ...
    // Validate API key
    if (!GEMINI_API_KEY) {
      console.error(`[Step: planning-query-enhancement] Missing GEMINI_API_KEY`);
    ...
    if (isSimpleQuery(originalQuery)) {
      console.log(`[Step: planning-query-enhancement] Query "${originalQuery}" is simple, skipping enhancement`);
    ...
    try {
      ...
      while (retryCount <= MAX_RETRIES) {
        try {
          ...
          retryCount++;
          console.error(`[Step: planning-query-enhancement] Retry ${retryCount}/${MAX_RETRIES}: ${error}`);
      ...
      try {
        ...
        try {
          ...
          console.error(`[Step: planning-query-enhancement] Error extracting JSON:`, extractError);
      ...
      console.log(`[Step: planning-query-enhancement] Enhanced query: "${parsedResponse.enhancedQuery}"`);
      ...
      console.error(`[Step: planning-query-enhancement] Error during query enhancement:`, error);
 ...
 */
function createSimplifiedEnhancement(query: string): z.infer<typeof planningOutput> {
  // Simple enhancements
